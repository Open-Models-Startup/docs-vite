...
description: &description "Projects based on PUBLISH can be configured in the following ways: 1. with project files (`package.json` or `.Qi X Labrc`), 2. with URL parameters#with-url-parameters."
head:
...
Projects based on PUBLISH can be configured in the following ways:
...
The `compileTrigger` option controls how file changes in the editor are written to the PUBLISH in-memory filesystem. Note that writing to this in-memory filesystem does not mean that changes are persisted on `qixlab.com` - this requires a separate “Save” action from users.
