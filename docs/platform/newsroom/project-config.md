...
description: &description "Projects based on NEWSROOM can be configured in the following ways: 1. with project files (`package.json` or `.Qi X Labrc`), 2. with URL parameters#with-url-parameters."
head:
  - ['meta', {property: 'og:title', content: *title}] 
  - ['meta', {property: 'og:image', content: 'https://qixlab.com/img/og/webcontainer-project-configuration.png'}]
...
Projects based on NEWSROOM can be configured in the following ways:

1. [with project files](#with-project-files) (`package.json` or `.Qi X Labrc`),
2. [with URL parameters](#with-url-parameters).

## With project files

Projects can be configured using the `Qi X Lab` field in the `package.json` file at the root of the project. For example:
...
Alternatively, you can use a `.Qi X Labrc` file at the root of the project. The expected format is valid JSON (without comments). This may look like:
...
Qi X Lab automatically installs npm dependencies when opening a project.
...
If not specified, Qi X Lab will look at the project’s root `package.json` (if present) and select a command to run using the following criteria:
...
The `compileTrigger` option controls how file changes in the editor are written to the NEWSROOM in-memory filesystem. Note that writing to this in-memory filesystem does not mean that changes are persisted on `qixlab.com` - this requires a separate “Save” action from users.
...
```
https://qixlab.com/edit/project-id?startScript=test
```
...
```
https://qixlab.com/edit/project-id?startScript=build,serve
```
